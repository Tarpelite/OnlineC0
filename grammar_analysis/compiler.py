...
class Compiler():
    ...
    def _error(self, msg):
        ...
        while (wd[2] != '专用符号' or wd[3] != ';') and (wd[2] != '专用符号' or wd[3] != ')') and (wd[2] != '专用符号' or wd[3] != '}') and self.words_p< self.words_lim:
            self._getword();
            wd = self._curword()
    ...
    def s_const_statement(self):
        ...
        if wd[2]!= '关键字' or wd[3] != 'CONST':
            ...
            if wd[2] != '专用符号' or wd[3] != ';':
                self.error_msg_box.append(["第"+str(wd[0])+'行缺少;'])
            else:
                self._getword()
